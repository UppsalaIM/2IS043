...
"""Top-level package for mkitlabs."""
...
def lab1_view_table_info():
    global bok_tweets
    return bok_tweets.info()

def lab1_filter_emojis():
...
def plot_top_words(num_word_instances, top_words):
...
def lab1_plot_top_words():
    global bok_tweets
    interact(plot_top_words, num_word_instances=widgets.IntSlider(min=1, value=50, continuous_update=False), top_words=widgets.IntSlider(min=1, value=30, continuous_update=False))
...
def lab1_plot_top_words_with_filters():
    global bok_tweets
...
def lab1_plot_top_words_with_custom_stopwords():
    global bok_tweets
    def create_term_document_matrix2(corpus, min_df=1):
        cvec = CountVectorizer(min_df=min_df, stop_words=tm.stopwords)
        tfmatrix = cvec.fit_transform(corpus)
